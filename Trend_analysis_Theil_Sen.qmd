...
author: BenS
date: "`r Sys.Date()`"
abstract: "Trend analysis using Environment Cantebrury data"
format:
  html:
    toc: true
    toc-location: left
...
The standard round function in R doesn't behave like others in excel so another function needs to be added. See [here](https://stackoverflow.com/questions/12688717/round-up-from-5) for more discussion on rounding in R.
...
# Can also use janitor:: function which implements the same round2 function but without defining it
...
```{r}
write.csv(
  pivot_wider(df_final_long, names_from = "name", values_from = "value"),
